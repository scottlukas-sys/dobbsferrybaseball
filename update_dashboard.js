...
function computePIS(playerStats) {
    ...
    function scoreGame(game) {
        ...
        if (h) {
            ...
            hitPts += (h.rbi || 0) * 0.75;      // run production (context-dependent, down from 1.5)
            hitPts += (h.r || 0) * 0.5;          // runs scored (context-dependent, down from 1.0)
            hitPts += (h.bb || 0) * 0.75;        // plate discipline / on-base
            hitPts += (h.hbp || 0) * 0.75;       // toughness + on-base
            hitPts += (h.sb || 0) * 0.5;          // speed/aggression
            hitPts -= (h.so || 0) * 0.25;         // unproductive AB penalty
            hitPts -= (h.e || 0) * 0.5;           // defensive liability penalty
            if (hits >= 2) hitPts += 1.5;          // multi-hit game bonus (down from 2.0)
        }
