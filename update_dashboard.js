...
const varsitySchedule = [
    ...
    { date: '2026-05-09', display: 'May 9', day: 'Sat', time: '4:30 PM', opponent: 'Tuckahoe', location: 'Away', venue: 'Somers High School', address: '120 Primrose St, Lincolndale, NY 10540', type: 'League' },
...
const jvSchedule = [
    ...
    { date: '2026-04-30', display: 'Apr 30', day: 'Thu', time: '4:30 PM', opponent: 'Rye Neck JV', location: 'Home', venue: 'Gould Park', address: '33 Ashford Ave, Dobbs Ferry, NY 10522', status: 'TBR' },
        ...
        if (g.status === 'TBR') {
            rows += `
                        <tr class="completed">
                            ...
                            <td>${g.opponent}</td>
                            <td>${g.location}</td>
                            <td><span class="game-badge" style="background-color:#c0392b;">To Be Rescheduled</span></td>
                        </tr>`;
            continue;
        }
...
//   Pitching (shared base): IP(1.5) + SO(1.0) - ER(1.25) - BB(0.5) - H(0.25)
//     JV:      W and SV dropped
//     Varsity: W(2) + SV(2)
//     Denominator: pitching appearances
...
function computePIS(playerStats) {
    ...
    function scoreGame(game, poolLabel) {
        ...
        if (p) {
            if (isJV) {
                // JV pitching: W and SV dropped (team stats / irrelevant at JV)
                pitPts += ipToInnings(p.ip || 0) * 1.5;   // depth = coach confidence
                pitPts += (p.so || 0) * 1.0;               // clean outs, bypass defense
                pitPts -= (p.er || 0) * 1.25;              // run prevention
                pitPts -= (p.bb || 0) * 0.5;               // free bases, 100% pitcher's fault
                pitPts -= (p.h || 0) * 0.25;               // contact allowed
            } else {
                // Varsity pitching: W/SV kept (meaningful at this level)
                pitPts += (p.w || 0) * 2;                  // wins matter at varsity (reduced from 3)
                pitPts += (p.sv || 0) * 2;                 // save = closed a tight game
                pitPts += ipToInnings(p.ip || 0) * 1.5;   // depth = coach confidence
                pitPts += (p.so || 0) * 1.0;               // clean outs, bypass defense
                pitPts -= (p.er || 0) * 1.25;              // run prevention
                pitPts -= (p.bb || 0) * 0.5;               // free bases, 100% pitcher's fault
    ...
    function processPool(pool, poolLabel) {
        for (const [name, data] of Object.entries(pool)) {
            ...
            // Combined PIS: use role-appropriate denominator for DF/JV; cumulative for opponents
            const pis = isOpponent ? Math.round((hitTotal + pitTotal) * 10) / 10 : (pitTotal > hitTotal ? pitPtsPerGame : hitPtsPerGame);
...
function buildPlayersToWatch(pisData) {
    ...
    function renderDFTile(p) {
        ...
        // Show role-specific per-game score on the badge (2 decimal)
